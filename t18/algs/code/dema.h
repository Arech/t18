		...
		namespace code {
				...
				, public _i::EMABase
			{
				...
				struct algState {
					...
					algState() noexcept : r1(tNaN<prm_gamma_t>), r2(tNaN<prm_gamma_t>) {}
				};


				template<typename ContDT, typename ContST>
				static void dema(ContDT& dest, const ContST& src, const prm_gamma_t gamma, const prm_gamma_t omgamma
					, algState& state, const bool bClose) noexcept
				{
					...
					if (UNLIKELY(ss < realMinSrcHist)) {
						...
						T18_ASSERT(isfinite(r1) && isfinite(r2));
